...
export default function ConciseRatingSummary({ onNavigate }) {
  ...
  return (
    <section className="py-16 bg-sand-100/90 text-slate-900 border-t border-sand-200 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-white border border-sand-300 rounded-2xl p-6 sm:p-8 shadow-md mb-10">
          
          <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
            <div className="flex items-center gap-3">
              <span className="text-4xl sm:text-5xl font-serif font-extrabold text-amber-600">4.9</span>
              <div>
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star key={star} className="w-4 h-4 fill-amber-400 text-amber-500" />
                  ))}
                </div>
            ...
            <div className="hidden sm:block w-px h-10 bg-sand-300" />

            <div className="flex items-center gap-2 text-xs text-slate-700">
              <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" />
              <span>
                <strong className="text-slate-900 font-bold">1,284+ Community Reviews</strong> across 29 Indian States
              </span>
            </div>
          </div>
          ...
          <button
            ...
            className="px-5 py-3 rounded-xl bg-terracotta-500 hover:bg-terracotta-600 text-white font-bold text-xs shadow-md flex items-center gap-2 transition-all hover:scale-105 shrink-0"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {TOP_FEATURED_REVIEWS.map((rev, idx) => (
            <motion.div
              ...
              className="bg-white border border-sand-300 rounded-2xl p-5 shadow-sm hover:border-sand-400 transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  ...
                  <span className="text-[10px] font-mono text-terracotta-700 font-semibold bg-terracotta-500/10 px-2 py-0.5 rounded border border-terracotta-500/20 flex items-center gap-1">
              ...
              <div className="pt-3 border-t border-sand-200 flex items-center justify-between text-[11px]">
                <span className="font-serif font-bold text-slate-900">{rev.name}</span>
