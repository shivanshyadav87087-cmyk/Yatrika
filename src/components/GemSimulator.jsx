...
import { destinationsDataset, getCitiesForState, getLandmarksForStateAndCity } from '../data/destinations';
...
export default function GemSimulator({ customGems }) {
  ...
  const filteredGems = useMemo(() => {
    ...
    if (selectedCity !== 'All Cities / Districts') {
      pool = pool.filter(g => (g.city && g.city.toLowerCase() === selectedCity.toLowerCase()) || g.location.toLowerCase().includes(selectedCity.toLowerCase()));
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-base sm:text-lg text-sand-700 leading-relaxed font-sans">
            Select an Indian State, City & Landmark (e.g. <strong className="font-bold text-slate-900">Taj Mahal, Amer Fort, Kashi Vishwanath, Munnar, Pangong</strong>)! Yatrika automatically displays <strong className="font-bold text-slate-900">uncrowded offbeat places nearby in that region</strong>, plus famous local food & native clothes—backed by 24/7 nearest police station mapping & emergency SOS support.
          </p>
        </motion.div>
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="relative">
                ...
                >
                  {availableLandmarks.length > 0 ? (
                    ...
                    <option value="all">No specific landmark data for this city yet</option>
                  )}
                </select>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {filteredGems.length > 0 ? (
                ...
                <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-xs font-sans text-center space-y-1">
                  ...
                  <span className="text-[11px] text-amber-700 block">You can submit a new hidden place for this state in the submission section below.</span>
                </div>
              )}
            </div>
          ...
          >
            ...
            <div className="bg-slate-900 text-white p-6 sm:p-8 rounded-3xl border border-slate-700 shadow-2xl relative min-h-[460px] flex flex-col justify-between overflow-hidden topo-pattern-dark">
              ...
              {!currentGem ? (
                /* Fallback Empty State for Unseeded State/City */
                <motion.div
